...
function executableProvidersSource(bindings: ExecutableBindingPlan): string {
  const imports = renderImports(bindings.providers.flatMap(providerBindingImports));
  const rows = bindings.providers.map((binding) => `  Object.freeze({
    ...
    ${binding.scope === "graph" || binding.scope === "request" ? `graphId: ${binding.graphId},\n    ` : ""}dependencyIds: Object.freeze(${JSON.stringify(binding.dependencyIds)}),
...
function httpBindingsSource(optimized: OptimizedApplication): string {
  ...
  // Every route calls prepareHttpValidationInput unconditionally — with flags 0 when
  // there's no validator, which is cheap/correct already since every branch in that
  // function is flag-gated — so every route builds a real AppRequest at runtime
  // (previously, no-validator routes skipped this and handlers received the raw
  // native Request instead, despite being typed as AppRequest).
  const paths = [...new Set(optimized.routes.map((route) => route.pathId))].sort((a, b) => a - b).map((pathId) => {
    const methods = optimized.routes.filter((route) => route.pathId === pathId).map((route) =>
      `    [strings[${route.methodId}]!]: (request: Request) => prepareHttpValidationInput(request, ${route.validatorId < 0 ? "0" : `validatorBindings[${route.validatorId}]!.flags`}).then((input) => executeHttpRoute(${route.id}, request, input)),`,
    ).join("\n");
    return `  [strings[${pathId}]!]: Object.freeze({\n${methods}\n  }),`;
  }).join("\n");
  return `/* Generated by @warbler/compiler. Do not edit. */
