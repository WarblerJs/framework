...
`).join("\n");
  const rows = bindings.handlers.map((binding) => `  Object.freeze({ id: ${binding.id}, controllerId: ${binding.controllerId}, parameterCount: ${binding.parameterCount}, invoke: invokeHandler${binding.id} }),`).join("\n");
  return `/* Generated by @warbler/compiler. Do not edit. */
