...
function resolveFrameworkValue(name: string, preferredFile: string | undefined, imports: Map<string, BindingImport>): BindingImport | undefined {
  if (preferredFile === "@warblerjs/websocket" && name === "SocketPublisher") return frameworkImport(name, "SocketPublisher", "@warblerjs/websocket", imports);
  if (preferredFile === "@warblerjs/events" && name === "EventDispatcher") return frameworkImport(name, "EventDispatcher", "@warblerjs/events", imports);
  if (preferredFile === "@warblerjs/email" && name === "Email") return frameworkImport(name, "Email", "@warblerjs/email", imports);
  return undefined;
}
function resolveFrameworkDeclaration(name: string, declaration: ts.Declaration, imported: string, imports: Map<string, BindingImport>): BindingImport | undefined {
  ...
  if (imported === "SocketPublisher" && source.endsWith("/packages/websocket/src/publisher.ts")) return frameworkImport(name, "SocketPublisher", "@warblerjs/websocket", imports);
  if (imported === "EventDispatcher" && source.endsWith("/packages/events/src/index.ts")) return frameworkImport(name, "EventDispatcher", "@warblerjs/events", imports);
  if (imported === "Email" && source.endsWith("/packages/email/src/email.ts")) return frameworkImport(name, "Email", "@warblerjs/email", imports);
  return undefined;
}
