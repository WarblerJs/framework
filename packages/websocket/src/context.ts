...
export interface SocketContext<TUser = unknown> {
  readonly connection: SocketConnection; readonly user?: TUser; readonly subscriptions: readonly string[]; readonly log: SocketLogger;
...
export function createSocketContext<TUser>(socket: NativeSocketLike, connection: SocketConnection, options: { readonly user?: TUser; readonly format?: SocketMessageFormat; readonly subscriptionLimit?: number; readonly logger?: SocketLogger } = {}): SocketContext<TUser> {
  ...
  return {
    ...
    send(message, sendOptions) { return interpretSendResult(socket.send(encodeSocketMessage(message as SocketOutgoingMessage<unknown>, format), sendOptions?.compress)); },
