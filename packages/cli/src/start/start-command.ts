...
export async function startCommand(layout: ProjectLayout, trailingArguments: readonly string[] = []): Promise<number> {
  ...
  if (!await pathExists(entry) || !await pathExists(manifest)) throw new CLIError("CLI3001", "Production build not found.", ExitCode.FAILURE, "Run: warbler build");
  const value = await readJson(manifest);
  if (value.entry !== "server.js" || value.version !== 1) throw new CLIError("CLI3002", "Production build manifest is invalid.", ExitCode.INVALID_PROJECT, "Run: warbler build");
