...
var finiteInteger = (value, name) => {
  if (!Number.isSafeInteger(value) || value < 0)
    throw new RangeError(`${name} must be a non-negative safe integer.`);
  return value;
};
var finiteNumber = (value, name) => {
  if (!Number.isFinite(value))
    throw new RangeError(`${name} must be finite.`);
  return value;
};
var empty = (value) => value === null || value === "" || value === false || Array.isArray(value) && value.length === 0;
var regexSource = (input) => {
...
var owners = new InstanceRegistry;
...
var loginForm = FormBuilder("register-form", (v) => ({
  email: [
    "bellib@hotmail.fr",
...
//# debugId=04D5AE9F0B35991164756E2164756E21
